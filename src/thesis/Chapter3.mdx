...
ordered list of vertices and edges) where: $$\begin{align}
...
\end{align}$$ $l_e$ are convex costs associated with each edge $e$. Here
...
\label{eq:objective}$$ Here $a$, $b$, and $c_n$ are user-specified
...
\label{eq:path_length_cost}$$ This provides an upper bound on path
...
\label{eq:path_derivative_cost}$$ This expression remains convex for
...
trajectory duration
[\[eq:time_cost\]](#eq:time_cost){reference-type="eqref"
reference="eq:time_cost"}, path length
[\[eq:path_length_cost\]](#eq:path_length_cost){reference-type="eqref"
reference="eq:path_length_cost"}, and smoothness
[\[eq:path_derivative_cost\]](#eq:path_derivative_cost){reference-type="eqref"
reference="eq:path_derivative_cost"}, tailoring the optimization to
...
\end{align}$$ However, constraints on higher-order derivatives, such as
...
\label{eq:nonlinear_acceleration}$$
...
envelope. The orange line shows a simpler
approximation.](figures/math/mcCormickEnvelope.png){#fig:mcCormick
width="40%"}

For example, consider a 1D path with acceleration $x$ and limits \[-1,
1\]. The nonlinear constraint is: $$-1 \leq \frac{x}{h^2} \leq 1.
\label{eq:simple_1d_acceleration}$$ The feasible set is highlighted in
gray in Figure [3.1](#fig:mcCormick){reference-type="ref"
reference="fig:mcCormick"}.
...
\label{eq:mcCormick_bound}$$ The single McCormick envelope using
[\[eq:mcCormick_bound\]](#eq:mcCormick_bound){reference-type="eqref"
reference="eq:mcCormick_bound"} is shown as a blue line. Alternatively,
...
\label{eq:linearized_acceleration_constraint}$$ where $h_0$ is a
...
constraints: $$\frac{d^Nr(s)}{ds^N} \in h^N \mathcal D.
\label{eq:derivative_constraint}$$
...
$$r_{k,d} = r_{k+1,0}.
\label{eq:zero_order_continuity}$$ This guarantees that the robot's path
...
curves: $$\label{eq:higher_order_continuity}
\frac{d^Nr_{k,d}}{ds^N}h_{k+1}^N = \frac{d^Nr_{k+1,0}}{ds^N}h_k^N.$$ We
introduce a convex surrogate by replacing the time scaling variables
with a constant value ($h_0$) for each region:
$$\label{eq:convex_higher_order_continuity}
...
### 3.2.4    Collision Avoidance {#section:collision_avoidance}
...
enforce the following constraint: $$\label{eq:min_distance}
...
[\[alg:max_dist_polyhedra\]](#alg:max_dist_polyhedra){reference-type="ref"
reference="alg:max_dist_polyhedra"}) to estimate the length of each
...
### 3.2.5    Task Space Constraints {#section:task_space_constraints}
...
\label{eq:path_position_constraint}$$ For common manipulators the
...
\label{eq:task_space_velocity_constraint}
J(r(s_i)) \dot{r}(s_i) &\in h\mathcal V, &&\forall s_i \in S_U,\\
\label{eq:task_space_acceleration_constraint}
J(r(s_i)) \ddot{r}(s_i) + h\dot{J}(r(s_i)) \dot{r}(s_i) &\in h^2\mathcal A, &&\forall s_i \in S_U.
\end{align}$$ where $\mathcal V, \mathcal A\subseteq \mathbb R^6$ are
...
![Figure 2: Sequential and parallel subgraphs in a hierarchical graph
...
the subgraphs.](figures/math/sequential_graph.png){#fig:sequential_graph
width="90%"}
...
We will use figure [3.2](#fig:sequential_graph){reference-type="ref"
reference="fig:sequential_graph"} to illustrate the potential of
...
### 3.3.3    Utilizing Subgraphs as Start and Goal Regions {#section:multi_start_and_goal}

![Figure 3: Hierarchical graph structure with subgraphs as start and
...
start or
target.](figures/math/multi_start_graph.png){#fig:multi_start_graph
width="90%"}
...
Figure [3.3](#fig:multi_start_graph){reference-type="ref"
reference="fig:multi_start_graph"} illustrates such a scenario, where
