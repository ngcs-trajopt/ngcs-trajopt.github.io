...
$$\begin{align} \label{eq:velocity_constraint} \dot r(s) \in h \mathcal V. \end{align}$$
...
$$\frac{d^N q(t)}{dt^N} = \frac{d^N r(s)/ds^N}{h^N}. \label{eq:nonlinear_acceleration}$$

<figure id="fig:mcCormick" style={{display:'flex', alignSelf: 'center', alignItems:"center", flexDirection: 'column'}}>
<Suspense fallback={null}>
<img style={{width:"300px"}} src="figures/math/mcCormickEnvelope.png"/>
</Suspense>
<figcaption>Figure 1: Convex Approximations of 1D acceleration limits. The filled
...
gray in Figure [3.1](#fig:mcCormick)
...
inequalities:
$$h^N \leq \frac{h_{max}^N - h_{min}^N}{h_{max} - h_{min}}h + \frac{h_{max}h_{min}^N - h_{min}h_{max}^N}{h_{max} - h_{min}}.\label{eq:mcCormick_bound}$$
...
$$\frac{d^N r(s)}{ds^N} \in h_0^{N-1} h \mathcal D,\label{eq:linearized_acceleration_constraint}$$
where $h_0$ is a
...
constraints: $$\frac{d^Nr(s)}{ds^N} \in h^N \mathcal D.\label{eq:derivative_constraint}$$
...
$$r_{k,d} = r_{k+1,0}.\label{eq:zero_order_continuity}$$ This guarantees that the robot's path
...
curves: $$\label{eq:higher_order_continuity} \frac{d^Nr_{k,d}}{ds^N}h_{k+1}^N = \frac{d^Nr_{k+1,0}}{ds^N}h_k^N.$$ We
introduce a convex surrogate by replacing the time scaling variables
with a constant value ($h_0$) for each region:
$$\label{eq:convex_higher_order_continuity} \frac{d^Nr_{k,d}}{ds^N}h_{0, k+1}^N = \frac{d^Nr_{k+1,0}}{ds^N}h_{0,k}^N.$$
...
enforce the following constraint: $$\label{eq:min_distance} \min_{x \in R_k(r(s_i)), y \in O_j} | x - y |_2 \geq d_{\text{min}}, \hspace{25pt} \forall (j, k) \in \mathcal{C}, \forall s_i \in S_I,$$
...
$$f_{kin}(r(s_i)) \in \relax, \hspace{35pt} \forall s_i \in S_U.\label{eq:path_position_constraint}$$
...
derivative: $$\begin{align} \label{eq:task_space_velocity_constraint} J(r(s_i)) \dot{r}(s_i) &\in h\mathcal V, &&\forall s_i \in S_U,\\ \label{eq:task_space_acceleration_constraint} J(r(s_i)) \ddot{r}(s_i) + h\dot{J}(r(s_i)) \dot{r}(s_i) &\in h^2\mathcal A, &&\forall s_i \in S_U. \end{align}$$
